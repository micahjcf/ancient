...
//****************************************************************
void SHA1Update(SHA1State* State, BYTE* M, UINT nBytes)
{
	if(! nBytes)
		return;
	...
	if (((DWORD)(State->Length_Low + ((DWORD)nBytes << 3))) < State->Length_Low)
		State->Length_High++;
...
//****************************************************************
void SHA1Result(SHA1State* State, BYTE hash[ SHA1HASHSIZE ])
{
	SHA1PadMessage(State);

	for(UINT i = 0; i < SHA1HASHSIZE; ++i)
		hash[i] = (BYTE)(State->Intermediate_Hash[i>>2] >> 8 * ( 3 - ( i & 0x03 ) ));
}
...
void SHA1ProcessMessageBlock(SHA1State* State)
{
	...
	for(t = 0; t < 16; t++)
	{
		W[t] = State->MsgBlock[t * 4] << 24;
		W[t] |= State->MsgBlock[t * 4 + 1] << 16;
		W[t] |= State->MsgBlock[t * 4 + 2] << 8;
		W[t] |= State->MsgBlock[t * 4 + 3];
	}
...
void SHA1PadMessage(SHA1State* State)
{
	...
	*/
	...
	if(shai > 55)
	{
		State->MsgBlock[shai++] = 0x80;

		while(shai < 64)
			State->MsgBlock[shai++] = 0;
	...
	State->MsgBlock[56] = (BYTE)(State->Length_High >> 24);
	State->MsgBlock[57] = (BYTE)(State->Length_High >> 16);
	State->MsgBlock[58] = (BYTE)(State->Length_High >> 8);
	State->MsgBlock[59] = (BYTE)(State->Length_High);
	State->MsgBlock[60] = (BYTE)(State->Length_Low >> 24);
	State->MsgBlock[61] = (BYTE)(State->Length_Low >> 16);
	State->MsgBlock[62] = (BYTE)(State->Length_Low >> 8);
	State->MsgBlock[63] = (BYTE)(State->Length_Low);
