...
#define sigma(x)	(x ^ (rotl(x, 13) + rotl(x, 27)))
...
{
	DWORD A, B, C, D, E, F, G, H, t, n, W[70], next( GetNextSquareFreeChar(&State->SequenceState) );
...
void MFAUpdate(MFAState* State, BYTE* M, UINT nBytes)
{
	...
	UINT MsgIndex = ((BYTE)(State->Length_Low >> 3)) % 80;
