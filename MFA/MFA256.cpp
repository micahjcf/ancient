...
void MFACompressG(MFAState* State)
{
	...
	// Message Expansion part 1
	for(n = 22; n < 46; n++)
		W[n] = rotl(W[n-1], 23) + (W[n-3] ^ W[n-7] ^ W[n-18]) + rotl(W[n-2] + W[n-22], 7);

	// Message Expansion part 2
	for(n = 46; n < 70; n++)
		W[n] = rotl(W[n-1], 7) + (W[n-6] ^ W[n-12] ^ W[n-21]) + rotl(W[n-2] + W[n-24], 27);
...
void MFACompressH(MFAState* State)
{
	...
	// Message Expansion part 1
	for(n = 22; n < 46; n++)
		W[n] = rotl(W[n-2], 29) + (W[n-4] ^ W[n-9] ^ W[n-15]) + rotl(W[n-1] + W[n-22], 5);

	// Message Expansion part 2
	for(n = 46; n < 70; n++)
		W[n] = rotl(W[n-24], 13) + (W[n-5] ^ W[n-10] ^ W[n-19]) + rotl(W[n-1] + W[n-2], 19);
...
{
	UINT next = GetNextSquareFreeChar(&State->SequenceState);
...
void MFAResult(MFAState* State, BYTE Hash[ MFAHashSize ])
{
	...
	for(n = 0; n < 8; n++)
	{
		Hash[n*4  ] = State->Intermediate_Hash[n] >> 24;
		Hash[n*4+1] = State->Intermediate_Hash[n] >> 16;
		Hash[n*4+2] = State->Intermediate_Hash[n] >> 8;
		Hash[n*4+3] = State->Intermediate_Hash[n];
	}
}
