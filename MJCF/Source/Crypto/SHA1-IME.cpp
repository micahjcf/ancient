...
void SHA1_IME_PadMessage(SHA1IMEState*);
void SHA1_IME_ProcessMessageBlock(SHA1IMEState*);

//****************************************************************
void SHA1_IME_Result(SHA1IMEState* State, BYTE Message_Digest[ SHA1HashSize ])
{
	UINT i;

	SHA1_IME_PadMessage(State);
...
//****************************************************************
void SHA1_IME_Update(SHA1IMEState* State, BYTE* message_array, UINT length)
{
	...
	while(length--)
	{
		...
		{
			shai = 0;
			SHA1_IME_ProcessMessageBlock(State);
		}
	}
}

//****************************************************************
void SHA1_IME_ProcessMessageBlock(SHA1IMEState* State)
{
	UINT t;
	...
	// Run IMPROVED message expansion
	for(t = 16; t < 36; t++)
		W[t] = W[t-3] ^ W[t-8] ^ W[t-14] ^ W[t-16] ^ rotl(W[t-1] ^ W[t-2] ^ W[t-15], 13);
	for(t = 36; t < 80; t++)
		W[t] = W[t-3] ^ W[t-8] ^ W[t-14] ^ W[t-16] ^ rotl(W[t-1] ^ W[t-2] ^ W[t-15] ^ W[t-20], 13);
...
//****************************************************************
void SHA1_IME_PadMessage(SHA1IMEState* State)
{
	/*
